...
<html>
<head>
<title>템플릿게시판 목록화면- (JqGrid)</title>
...
<link rel="stylesheet" type="text/css"
	href="/css/common/jquery/jquery.ui.all.css" />
<link rel="stylesheet" type="text/css"
	href="/css/common/jquery/layout-default-latest.css" />
<link rel="stylesheet" type="text/css"
	href="/css/common/jquery/jquery.ui.theme.css" />
<%-- stylesheet End	========================================================--%>
...
function fn_selective_delete()
{
	...
	if (confirm("<spring:message code="common.delete.msg" />")) {
   		JQ_request("searchVO", "<c:url value='/sample/tem/deleteTemplateJqGridList.do'/>");
	}
}
...
function link_page(pageNo){
	...
	JQ_request("searchVO", "<c:url value='/sample/tem/selectTemplateJqGridList.do'/>");
}
...
function fn_search(){
	document.listForm.pageIndex.value = 1;
   	JQ_request("searchVO", "<c:url value='/sample/tem/selectTemplateJqGridList.do'/>", "searchVO");
}
 ...
 ************************************************************************/
 --%>
 function fn_regist(){
   	JQ_request("searchVO", "<c:url value='/sample/tem/registerTemplateJqGrid.do'/>", "searchVO");
}
...
function fn_detail(templateId){
 	JQ_setValueObj('templateId', templateId); 
   	JQ_request("searchVO", "<c:url value='/sample/tem/selectTemplateJqGridDetail.do'/>", "searchVO");
}
...
<div class="page_title">게시판 - (JqGrid)</div>
<div class="frmctl" id="frmctl"></div>
</div>
<!-- page title / frameset controller end -->

<div class="center_inner_contents" id="contents_area"><form:form
	commandName="searchVO" name="listForm" method="post">
			...
			<table class="title">
				<tr>
					...
					<table class="inner_btn">
						<tr>
							<td><img src="/images/button/0428.png"
								alt="<spring:message code="button.selectedToDelete" />"
								title="<spring:message code="button.selectedToDelete" />" style="cursor:hand"
								onclick="fn_selective_delete()"></td>
							<td><img src="/images/button/0613.png"
								alt="<spring:message code="button.exceldownload" />"
								title="<spring:message code="button.exceldownload" />" style="cursor:hand"
								onclick="fn_excel_download()"></td>
	...
	<table class="default">
				...
				<tr>
					...
					<table class="option">
						<tr>
							...
							<td><form:select path="searchCondition">
								<form:option value="" label="선택하세요" />
								<form:options items="${searchCondition}" />
							</form:select></td>
							<td><input name="searchKeyword" type="text" size="35"
								value="${searchVO.searchKeyword}" maxlength="35"
								id="searchKeyword" 
								onfocus="gfn_focus(this);" onblur="gfn_focusnot(this);"></td>
			             ...
			             --> 
			                 <form:input path="fromDate" cssStyle="vertical-align:middle"  size="10"
								cssClass="input_t input_nara_date" 
								onfocus="gfn_focus(this);" onblur="gfn_focusnot(this);"/> ~ 
							<form:input 
								path="toDate" cssStyle="vertical-align:middle"  size="10" cssClass="input_t input_nara_date"
								onfocus="gfn_focus(this);" onblur="gfn_focusnot(this);"/></td>
							<td><img src="/images/button/0221.png"
								alt="<spring:message code='button.inquire'/>"
								title="<spring:message code='button.inquire'/>"								 
								style="cursor:hand"
								onclick="fn_search()"></td>
							<td><img src="/images/button/0202.png"
								alt="<spring:message code='button.create'/>"
					...
					<dl class="board_counter">
						...
						<dd class="total"><c:out
							value='${paginationInfo.totalRecordCount}' /></dd>
						<dt class="page"><spring:message code='common.template.page'/></dt>
						<dd class="page"><c:out
							value='${paginationInfo.currentPageNo}' /> / <c:out
							value='${paginationInfo.totalPageCount }' /></dd>
					</dl>
					</td>
				</tr>
			</table>
		...
		<tr>
			...
			<table class="datalist01" summary="게시판 템플릿 입니다..">
				...
				<thead>
					<tr>
						<th scope="col" width="3%"><input type="checkbox"
							id="selectall" class="check" title="전체선택"></th>
				...
				<tbody>
					<c:forEach items="${resultList}" var="resultInfo"
						...
						<tr onMouseDown="this.style.background='#AFE8F7'"  onMouseOver="this.style.background='#B5C8EF'" onmouseout="this.style.background=''">
							<td class="column3"><input type="checkbox" name="chkbox"
								value="${resultInfo.templateId}" /></td>
							...
							<td class="column3" style="cursor: hand"
								onclick="javascript:fn_detail('${resultInfo.templateId}');">${resultInfo.templateTitle}</td>
		...
		<tr>
			...
			<table class="datapaging" align="center">
				<tr>
					<td class="tc"><ui:pagination
						paginationInfo="${paginationInfo}" type="image"
						jsFunction="link_page" /> <input name="pageIndex" type="hidden"
						value="<c:out value='${searchVO.pageIndex}'/>" /></td>
				</tr>
			</table>
			</td>
		</tr>
