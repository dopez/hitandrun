		...
		<table class="default" width="${bbsInfoVO.tableSize}" summary="${bbsInfoVO.bbsNm}게시판 정보">
			...
			<tr>
				<td>
									...
									<tr>
										<td class="tl">
											...
											<tr>
								    			...
								    			<td><img src="/images/button/0208.gif" alt="<spring:message code='button.search'/>" title="<spring:message code='button.search'/>" style="cursor:hand;" onclick="fn_search()"></td>
								    			<td><a href="<c:url value="/gps/bbs/selectBbsList.do">
								    							<c:param name="bbsId" value="${bbsSearchVO.bbsId}"/>
								    							<c:param name="menuId" value="${bbsSearchVO.menuId}"/>
								    							<c:param name="leftMenuId" value="${bbsSearchVO.leftMenuId}"/>
								    						</c:url>">
								    				<img src="/images/button/0318.png" alt="<spring:message code='button.initialization'/>" title="<spring:message code='button.initialization'/>"></a></td>
											</tr>
											</table>
										</td>
										<td>
					...
					<tr>
						<td>
							<table class="boardPhotoList" width="${bbsInfoVO.tableSize}" summary="${bbsInfoVO.bbsNm}에 ${paginationInfo.totalRecordCount}개의 게시물이 있습니다.">
							<caption>${bbsInfoVO.bbsNm} 게시물 입니다.</caption>
							<tr>
								<td>
									<table class="default" summary="${bbsInfoVO.bbsNm}에 ${paginationInfo.totalRecordCount}개의 게시물이 있습니다.">
									<caption>${bbsInfoVO.bbsNm} 게시물 입니다.</caption>
									<tr>
									<c:set var="titleLimit" value="16"/>
										<td>
											<table class="default" summary="${bbsInfoVO.bbsNm} 게시물이 입니다.">
											<caption>${bbsInfoVO.bbsNm} 게시물 입니다.</caption>
												<c:out value="<tr>" escapeXml="false"/>
												<c:forEach var="list" items="${bbsList}" varStatus="status">
													<td>
														<table class="boardInside">
															<tr>
																<td class="photo">
																	<a href="#LINK" onclick="fn_detail('<c:out value="${list.bbsSn}"/>');return false;" onkeypress="this.onclick();return false;">
																		<c:choose>
																			<c:when test="${!empty list.imageFileId}">
																				<img src="<c:url value="/cmm/fms/getImage.do"><c:param name="atchFileId" value="${list.imageFileId}"/><c:param name="fileSn" value="0"/></c:url>" alt="<c:out value="${list.sj}"/>" title="<c:out value="${list.sj}"/>" width="200"/>
																			</c:when>
																			<c:otherwise>
															...
															<tr>
																<td>
																	<table class="subject" summary="${list.sj}입니다.">
																	<caption>${list.sj} 게시물 입니다.</caption>
																	<tr>
																		<th>제목</th>
																		<td>
																			...
																			<a href="#LINK" onclick="fn_detail('<c:out value="${list.bbsSn}"/>');return false;" onkeypress="this.onclick();return false;"><c:out value="${list.sj}" escapeXml="false"/></a>
																		</td>
																	</tr>
																	<tr>
																		<th>등록일</th>
																		<td class="f_eng11"><c:out value="${list.registDt}"/></td>
																	</tr>
																	<tr>
																		<th>조회</th>
																		<td class="f_eng11">${list.co}</td>
																	</tr>
																	</table>
																</td>
															</tr>
													...
													<c:if test="${status.count % bbsInfoVO.albumColumnCo == 0 }">
														<c:out value="</tr>" escapeXml="false"/>
														<c:out value="<tr>" escapeXml="false"/>
													</c:if>
													<c:if test="${fn:length(bbsList) == status.count}">
														<c:out value="</tr>" escapeXml="false"/>
													</c:if>
												</c:forEach>
												<c:if test="${empty bbsList}">
													<tr> 
														<td align="center" colspan="${bbsInfoVO.albumColumnCo}">
