<%-- 
/** 
 * 설명   : 메인화면에서 만족도 조사를 표시한다.
 * 파일명 : satisfactionRspns.jsp
 * @author 범정부통계포털 이관형 
 * @since 2011.06.20 
 * @version 1.0 
 * @see 
...
function fn_selective_delete() {
	...
	if (confirm("<spring:message code="common.delete.msg" />")) {
   		JQ_request("csnstMenoVO", "<c:url value='/gps/csnst/deleteSatisfactionMemoList.do'/>");
	}
}
...
function link_page(pageNo){
	...
	JQ_request("csnstMenoVO", "<c:url value='/gps/csnst/satisfactionRspnsResult.do'/>");
}
				...
				<tr>
					...
					<c:forEach var="resultInfo" items="${memoList}" varStatus="status">
						<tr onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor=''">
							<td class="column10"><input type="checkbox" name="chkbox" title="<c:out value="${resultInfo.memoCn}"/>" value="${resultInfo.memoSn}" class="noline"/></td>
