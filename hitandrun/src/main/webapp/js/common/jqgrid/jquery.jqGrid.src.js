...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var grid={
			...
			/* MultiHeader 사용시 CellIndex 추출 */
			getMultiRowCellIndex : function (id) {
				...
				if( idx > -1 )
				{
					id = id.substr(idx+1, id.length);
				}

				for( i = 0; i <  c.headerSpanModel.length; i++ )
				{
					if( c.headerSpanModel[i].index == id ) 
					{
						arrcols[0] = c.headerSpanModel[i].sortindex;
						arrcols[1] = c.headerSpanModel[i].colindex;
					}
				}
				return arrcols;
			},
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if(locdata || ts.p.pagesort ) { 
				ts.p.data = []; ts.p._index = {}; ts.p.localReader.id = locid;
			}
			ts.p.reccount = 0;
			...
			while (i<len) {
				...
				if(locdata || ts.p.treeGrid===true  || ts.p.pagesort) {
					rd[locid] = idr;
					ts.p.data.push(rd);
		...
		addLocalData = function( headclick ) {
		...
		populate = function (npage, headclick ) {
			if(!ts.grid.hDiv.loading) {
				...
				var prevdatatype = ts.p.datatype; 
				
				if( headclick == true ) dt = "local";
				...
				case "clientside":
					...
					beginReq();
					ts.p.datatype = "local";
					var req = addLocalData( headclick );
				...
				if( headclick == true ) ts.p.datatype = prevdatatype;
		...
		sortData = function (index, idxcol,reload,sor, headclick){
			...
			if( headclick == true) ts.p.prevpage = ts.p.page; 
		...
		/* MultiHeader 처리 */		
		var arrheadertrs = new Array(this.p.headerDepth);
		if( this.p.headerSpanModel.length == 0 || this.p.headerDepth == 0  ) {
			...
			if ( this.p.headerSpanModel.length != 0 || this.p.headerDepth != 0 )
			{
				...
				for( i = 0; i < arrheadertrs.length; i++ )
				{
					arrheadertrs[i] = "<tr class='ui-jqgrid-labels' role='rowheader'>";
				}

				for( i = 0; i < this.p.headerSpanModel.length; i++ )
				{
					rowidx = this.p.headerSpanModel[i].rowindex;
					colindex = this.p.headerSpanModel[i].colindex;
						
					var tooltip = this.p.headertitles ? (" title=\""+$.jgrid.stripHtml(ts.p.colNames[colindex])+"\"") :"";
					

					if( this.p.headerSpanModel[i].colspan > 1 )
					{
						arrheadertrs[rowidx] += "<td style='text-align:center;' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir;
					}
					else
					{
						arrheadertrs[rowidx] += "<th id='"+ts.p.id+"_"+ts.p.colModel[colindex].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir;						
					}
					arrheadertrs[rowidx] += "'" + tooltip+" rowspan='" + this.p.headerSpanModel[i].rowspan + "'  colspan='" + this.p.headerSpanModel[i].colspan + "'";
					arrheadertrs[rowidx] += " >";

					idn = ts.p.colModel[colindex].index || ts.p.colModel[colindex].name;
					

					if( this.p.headerSpanModel[i].colspan > 1 )
					{
						arrheadertrs[rowidx] += this.p.headerSpanModel[i].headertitle;
						arrheadertrs[rowidx] += "</td>";
					}
					else
					{
						arrheadertrs[rowidx] += "<div id='jqgh_"+ts.p.colModel[colindex].name+"' "+tdc+">";
						arrheadertrs[rowidx] += ts.p.colNames[colindex];
						if(!ts.p.colModel[colindex].width)  { ts.p.colModel[colindex].width = 150; }
						else { ts.p.colModel[colindex].width = parseInt(ts.p.colModel[colindex].width,10); }
						if(typeof(ts.p.colModel[colindex].title) !== "boolean") { ts.p.colModel[colindex].title = true; }
						if (idn == ts.p.sortname) {
							ts.p.lastsort = i;
						} 
						arrheadertrs[rowidx] += imgs+"</div>";
						arrheadertrs[rowidx] += "</th>";
						
					}					
				}
				for( i = 0; i < arrheadertrs.length; i++ )
				{
					thead += arrheadertrs[i] + "</tr>";
				}				
			}
		}
		thead += "</thead>";
		...
		{	
			...
			for( i = 0; i < this.p.headerSpanModel.length; i++ )
			{
				rowidx = this.p.headerSpanModel[i].rowindex;
				colindex = this.p.headerSpanModel[i].colindex;
				rowspan = this.p.headerSpanModel[i].rowspan;
				colspan = this.p.headerSpanModel[i].colspan;
				
				if( colspan > 1 )
					continue;
				
				ctlid = "#" + ts.p.id + "_" + ts.p.colModel[colindex].index;		
								
				$(ctlid).each( function(j) {
					sa = ts.p.colModel[colindex].width;	
					if(typeof ts.p.colModel[colindex].resizable== "undefined")
						ts.p.colModel[colindex].resizable=true;
					if(ts.p.colModel[colindex].resizable){
						qa=document.createElement("span");
						$(qa).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+dir);
						$.browser.opera||$(qa).css("cursor","col-resize");
						$(this).addClass(ts.p.resizeclass);				
					}
					else 
						qa="";
					
					$(this).css("width",sa+"px").prepend(qa);
					var d="";
					if(ts.p.colModel[colindex].hidden){
						$(this).css("display","none");
						d="display:none;"
					}
					firstr+="<td role='gridcell' style='height:0px;width:"+sa+"px;"+d+"'></td>";
					this.headers[colindex]={width:sa,el:this};
					
					ta=ts.p.colModel[colindex].sortable;
					if(typeof ta!=="boolean") {
						ta=ts.p.colModel[colindex].sortable=false;
					}
					
					d=ts.p.colModel[colindex].name;
					
					if( !(d == 'cb' || d=='subgrid' || d=='rn') ) {
						if(ts.p.viewsortcols[2]){
							$("div",this).addClass('ui-jqgrid-sortable');
						}
					}
					//d=="cb"||d=="subgrid"||d=="rn"||ts.p.viewsortcols[2]&&$("div",this).addClass("ui-jqgrid-sortable");
					if(ta)if(ts.p.viewsortcols[0]){
						$("div span.s-ico",this).show();
						initwidth==ts.p.lastsort&& $("div span.ui-icon-"+ts.p.sortorder,this).removeClass("ui-state-disabled");
					}else if(colindex==ts.p.lastsort){
						$("div span.s-ico",this).show();
						$("div span.ui-icon-"+ts.p.sortorder,this).removeClass("ui-state-disabled");
					}
					if(ts.p.footerrow)
						tfoot+="<td role='gridcell' "+formatCol(colindex,0,"",null,"",false)+">&#160;</td>";
		...
		$("th",thr).each(function ( j ) {
			...
			if( typeof sort !== 'boolean') {				
				ts.p.colModel[j].sortable =  false; sort=false;
			}
			...
			var headclick = false;
			if( ts.p.pagesort ) headclick = true;
			//MultiHeader
			if( ts.p.headerDepth > 0 )
			{
				
				var ci = $.jgrid.getMultiRowCellIndex($('div',this)[0].id);

				if (!ts.p.viewsortcols[2]) { r=true;d=t.attr("sort"); }
				
				sortData( $('div',this)[0].id, ci, r, d, headclick );				
...
$.jgrid.extend({
	...
	downArrowCell : function (iRow,iCol) {
		return this.each(function (){
			...
			if( currpage == totpage )
			{
				currpagerecord = records - (page * rownum);
			}
			else
			{
