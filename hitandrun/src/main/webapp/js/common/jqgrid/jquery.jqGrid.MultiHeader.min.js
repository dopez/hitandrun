	...
	return this.each( function() {
		...
		if( this.p.headerSpanModel.length == 0 || this.p.headerDepth == 0  ) {
			...
			if ( this.p.headerSpanModel.length != 0 || this.p.headerDepth != 0 )
			{
				...
				{
					arrheadertrs[i] = "<tr class='ui-jqgrid-labels' role='rowheader' style='height:auto;'>";
				}
				...
				for( i = 0; i < this.p.headerSpanModel.length; i++ )
				{
					...
					if( this.p.headerSpanModel[i].colspan > 0 )
					{	
						arrheadertrs[rowidx] += "<th id='"+ts.p.id+"_"+this.p.headerSpanModel[i].index+"'  role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir;
						arrheadertrs[rowidx] += "'" + tooltip+" rowspan='" + this.p.headerSpanModel[i].rowspan + "'  colspan='" + this.p.headerSpanModel[i].colspan + "'";
						arrheadertrs[rowidx] += "style='height:30px;border:1px solid #afafaf;'";
					}
					else
					{
					...
					if( this.p.headerSpanModel[i].colspan > 0 )
					{
						...
						arrheadertrs[rowidx] += "<div id='jqgh_"+this.p.headerSpanModel[i].index+"' "+tdc+">";
						arrheadertrs[rowidx] += this.p.headerSpanModel[i].headertitle;
				...
				$(ctlid).each( function(j) {
					...
					if( !(d == 'cb' || d=='subgrid' || d=='rn') ) {
						if(ts.p.viewsortcols[2]){
							...
							$("div",this).css("word-wrap","break-word");
							$("div",this).css("width",sa+"px");
							$("div",this).css("height", "auto");
							
							//$("div",this).attr(style='white-space:pre-line;overflow: hidden;'";
						}
					}
		...
		if( ts.p.headerSpanModel.length != 0 || ts.p.headerDepth != 0 )
		{	
			hTable = $("<table style='width:"+ts.p.tblwidth+"px;' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead);
		}
		else
		{
			hTable = $("<table class='ui-jqgrid-htable' style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead);
		}
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			$(ts).jqGrid("setGridParam", {noRecordChk:false});
	...
	setSelection : function(selection,onsr) {
		return this.each(function(){
			...
			if($t.p.multiselect && $t.p.oneSelect){				
				...
				if(tpsr===-1 ){
					pt.className!=="ui-subgrid" && $(pt).addClass("ui-state-highlight").attr("aria-selected","true");
					stat=true;
		...
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				if ($.inArray(this.name,colname) !== -1 && this.hidden === sw) {
					...
					if( $t.p.headerDepth > 0 )
					{	
						
						$($t.p.headerSpanModel).each(function(idx) {
							if( i > idx )
							{
								if( h.p.headerSpanModel[idx].rowindex === 0 )
								{
									ia++;
								}
							}
						});
						$("tr:first",$t.grid.hDiv).each(function(){
			...
			if(fndh===true) {
				if($t.grid.width!==$t.p.tblwidth) {
                    if( $t.p.initWidth != 0 ) {
                    	$($t).jqGrid("setGridWidth", $t.p.initWidth, false );
                    } else {                    	
	...
	checkValues : function(val, valref,g, customobject, nam) {
		...
		if(edtrul) {
			...
			if(typeof edtrul.minValue != 'undefined' && !isNaN(edtrul.minValue)) {
				if (parseFloat(val) < parseFloat(edtrul.minValue) ) { return [false,nm+": "+edtrul.minValue+" "+$.jgrid.edit.msg.minValue,""];}
			}
			if(typeof edtrul.maxValue != 'undefined' && !isNaN(edtrul.maxValue)) {
				if (parseFloat(val) > parseFloat(edtrul.maxValue) ) { return [false,nm+": "+edtrul.maxValue+" "+$.jgrid.edit.msg.maxValue,""];}
			}
			var filter;
...
$.jgrid.extend({
	...
	noRecords:function() {
		return this.each(function(){
			var c=this;
			var cml;
			if( $(c).jqGrid("getGridParam", "noRecordText") && !$(c).jqGrid("getGridParam", "noRecordChk") && $(c).getGridParam("records") === 0)
			{	
				$(c).jqGrid("setGridParam", {noRecordChk:true});
				$(c).jqGrid("addRowData", -1, {});
				var cml = $(c).getGridParam("colModel").length;
				var rows = $('tbody tr', c).filter(':visible:not(.jqgfirstrow)');   // 저장 대상 row
				var col = $('td:eq(0)', rows[0]);
				
				$(col).attr("colspan", cml + 1);
				$(col).html('<b><center>조회된 데이타가 없습니다.</center></b>');	
				//b(c).setGridParam({records:0});
			}
			else if( $(c).jqGrid("getGridParam", "noRecordText") && $(c).jqGrid("getGridParam", "noRecordChk") && $(c).getGridParam("records") > 1 )
			{
				$(c).jqGrid("delRowData", -1);
			}			
		});
				...
				if ($t.p.colModel[iCol].editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
					...
					$("input, select, textarea",cc).bind("keydown",function(e) {						
						...
						if (e.ctrlKey==true && (e.which == '118' || e.which == '86')) {							
							...
							Paste($t.p.id, iRow, iCol, $t.grid.cols.length); 
						}
						if (e.keyCode === 27) {
